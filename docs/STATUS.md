...
- **uiohook-napi nach Multi-Arch-Build**: `electron-builder --mac` (ohne Arch-Filter) baut arm64 UND x64 und ruft `@electron/rebuild` jeweils auf. Das hinterlässt eine architekturfremde Binary in `node_modules/uiohook-napi/build/Release/`, die beim nächsten `npm run dev` Vorrang vor den Prebuilds bekommt und mit `ERR_DLOPEN_FAILED` crasht. Fix: `npm run clean:native` (entfernt das `build/`-Verzeichnis, Loader fällt auf den korrekten Prebuild zurück). Wird von `pack`, `dist`, `dist:mac` und `release.sh` automatisch nachgezogen.
- **Tray-Icon-Generator**: `scripts/gen-tray-icon.py` baut 22 px + 44 px Template-PNGs mit reiner Python-Stdlib. PIL ist auf macOS nicht vorinstalliert; manuelles PNG-Bauen über `struct`+`zlib` reicht für das einfache Pill-Glyph.
